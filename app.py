...
#load the sensitive values into app.py from .env
load_dotenv()

def get_llm_response(query,age_option,task_type_options):
    ...
    prefix = """You are a {template_age_option}, and {template_task_type_options}:
    ...
    new_prompt = FewShotPromptTemplate( 
        ...
        prefix=prefix,
        suffix=suffix,
        input_variables=["template_userInput","template_age_option","template_task_type_options"],
        example_separator="\n\n")

    print(new_prompt.format(template_userInput = query, template_age_option = age_option, 
                            template_task_type_options = task_type_options))

    print(llm(new_prompt.format(template_userInput = query,template_age_option = age_option, 
                            template_task_type_options = task_type_options)))
    
    response = llm(new_prompt.format(template_userInput = query,template_age_option = age_option, 
                            template_task_type_options = task_type_options))
    return response
...
task_type_options = st.selectbox("Please select the action to be performed?",
...
number_of_words = st.slider("word limit",
                            1,200,25)

submit = st.button("generate!")

if submit:
    st.write(get_llm_response(form_input, age_option, task_type_options))
